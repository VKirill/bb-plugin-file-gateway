...
test('tree targets selected host and inserts native mention without replacing draft',async()=>{
 ...
 fireEvent.click(await slot.findByRole('button',{name:'Add to chat: OVH:/shared/a [1].txt'}));
...
test('connection form sends password only on save and never renders stored secret',async()=>{
 ...
 fireEvent.click(slot.getByRole('button',{name:'Add connection'}));
 fireEvent.change(slot.getByLabelText('Name'),{target:{value:'Site'}});fireEvent.change(slot.getByLabelText('Server address'),{target:{value:'ftp.example.com'}});fireEvent.change(slot.getByLabelText('Username'),{target:{value:'user'}});fireEvent.change(slot.getByLabelText('Password'),{target:{value:'fixture-only'}});
 fireEvent.click(slot.getByRole('button',{name:'Save'}));await waitFor(()=>expect(saved.password).toBe('fixture-only'));expect(saved.connection.protocol).toBe('ftps');await slot.findByText('Connection saved');expect(slot.queryByLabelText('Password')).toBeNull();
});

test('header folder icon opens native right panel',async()=>{const app=await loadPluginApp(()=>import('../app.js'));const slot=renderSlot(app.threadHeaderActions[0],{threadId:'thread',projectId:'project',isCompactViewport:false});fireEvent.click(slot.getByRole('button',{name:'Open connected files'}));expect(slot.navigateCalls).toEqual([{method:'openThreadPanel',options:{actionId:'explorer',title:'Connected Files'}}]);});

test('hidden files and directories toggle without fetching contents or nested folders',async()=>{const app=await loadPluginApp(()=>import('../app.js'));let calls=0;const slot=renderSlot(app.threadPanelActions.find(a=>a.id==='explorer')!,{threadId:'thread',params:null},{rpc:{machines:()=>[{id:'mini',name:'Mini',status:'connected',roots:['/shared'],configured:true}],list:()=>{calls++;return {path:'/shared',entries:[{name:'.cache',kind:'directory'},{name:'.env',kind:'file'},{name:'public',kind:'directory'}],nextOffset:null};}}});await slot.findByRole('button',{name:'public'});expect(slot.queryByRole('button',{name:'.cache'})).toBeNull();expect(slot.queryByText('.env')).toBeNull();fireEvent.click(slot.getByRole('checkbox',{name:'Show hidden files and folders'}));expect(slot.getByRole('button',{name:'.cache'}).getAttribute('aria-expanded')).toBe('false');expect(slot.getByText('.env')).toBeTruthy();expect(calls).toBe(1);fireEvent.click(slot.getByRole('checkbox',{name:'Show hidden files and folders'}));expect(slot.queryByText('.env')).toBeNull();expect(calls).toBe(1);});
