...
function Folder({machine,path,label,initialOpen=false,showHidden}:{machine:Machine;path:string;label:string;initialOpen?:boolean;showHidden:boolean}){
 const rpc=useRpc<typeof explorerContract>();const composer=useComposer();const navigate=useBbNavigate();
 ...
 async function preview(target:string){setError('');try{const ref=await rpc.call('preview',{hostId:machine.id,path:target});if(!navigate.experimental_openFilePreview({target:{kind:'host',...ref},location:null}))throw new Error('Не удалось открыть просмотр файла');}catch(e){setError(e instanceof Error?e.message:String(e));}}
 ...
 return <div className="min-w-0">
  <div className="group flex h-8 items-center rounded-md pr-1 hover:bg-state-hover focus-within:bg-state-hover" title={path} draggable onDragStart={e=>beginFileDrag(e,machine.id,path,machine.name)} onDragEnd={endFileDrag}>
   ...
   <div className={rowActions}>{initialOpen&&<Button size="sm" variant="ghost" className="size-6 p-0" disabled={busy} aria-label={`Обновить папку ${path}`} onClick={()=>void load()}><ExplorerIcon name="refresh" className={`size-3.5 ${busy?'animate-spin':''}`}/></Button>}<Button size="sm" variant="ghost" className="size-6 p-0" aria-label={`В чат: ${machine.name}:${path}`} onClick={()=>add(path)}><ExplorerIcon name="plus" className="size-3.5"/></Button></div>
  </div>
  {open&&<div className="ml-3.5 border-l border-border/50 pl-2">
   {visible.map(entry=>{const target=`${path==='/'?'':path}/${entry.name}`;return entry.kind==='directory'?<Folder key={target} machine={machine} path={target} label={entry.name} showHidden={showHidden}/>:<div key={target} className="group flex h-8 min-w-0 items-center gap-2 rounded-md pl-6 pr-1 hover:bg-state-hover focus-within:bg-state-hover" title={target} draggable={entry.kind==='file'} onDragStart={e=>beginFileDrag(e,machine.id,target,machine.name)} onDragEnd={endFileDrag} onDoubleClick={()=>{if(entry.kind==='file')void preview(target);}}><ExplorerIcon name={fileIcon(entry.name)} className="size-4 text-muted-foreground"/><span className="min-w-0 flex-1 truncate text-sm">{entry.name}</span>{entry.kind==='file'&&<div className={rowActions}><Button size="sm" variant="ghost" className="size-6 p-0" aria-label={`В чат: ${machine.name}:${target}`} onClick={()=>add(target)}><ExplorerIcon name="plus" className="size-3.5"/></Button></div>}</div>;})}
   {error&&<div className="space-y-1 p-2"><p role="alert" className="break-words text-xs text-destructive">{error}</p><Button size="sm" variant="ghost" onClick={()=>void load()}>Повторить</Button></div>}
   {busy&&<div className="space-y-2 px-6 py-3" aria-label="Загрузка папки"><div className="h-3 w-2/3 animate-pulse rounded bg-muted"/><div className="h-3 w-1/2 animate-pulse rounded bg-muted"/></div>}
   {loaded&&!visible.length&&!busy&&<p className="px-6 py-3 text-xs text-muted-foreground">{entries.length?'Скрытые элементы не показаны':'Папка пуста'}</p>}
...
export function GatewayHeaderButton(){
 ...
 return <Button type="button" variant="ghost" size="sm" className="size-7 p-0" aria-label="Открыть файлы подключений" onClick={()=>navigate.openThreadPanel({actionId:'explorer',title:'Файлы подключений'})}><ExplorerIcon name="folder" className="size-4"/></Button>;
}
export function GatewayExplorer(){
 ...
 return <div className="flex h-full flex-col" aria-label="Файлы подключений">
  <div className="shrink-0 space-y-3 border-b border-border px-3 pb-3 pt-2">
   <div className="flex items-center gap-1"><Select value={selected} onValueChange={setSelected}><SelectTrigger aria-label="Сервер или FTP-подключение" className="h-9 min-w-0 flex-1 border-transparent bg-muted/40 px-2 hover:bg-state-hover"><span className="flex min-w-0 items-center gap-2"><ExplorerIcon name={selected.startsWith('remote_')?'globe':'server'} className="size-4 text-muted-foreground"/><SelectValue placeholder="Выберите подключение"/></span></SelectTrigger><SelectContent>{machines.map(m=><SelectItem key={m.id} value={m.id}>{m.name}{!m.configured?' — выключено':!['connected','configured'].includes(m.status)?' — не в сети':''}</SelectItem>)}</SelectContent></Select><Button size="sm" variant="ghost" className="size-8 shrink-0 p-0 text-muted-foreground" aria-label="Обновить подключения" onClick={()=>setVersion(v=>v+1)}><ExplorerIcon name="refresh"/></Button></div>
   <div className="flex flex-wrap items-center justify-between gap-x-3 gap-y-2 px-1"><span className="text-xs text-muted-foreground">＋ в чат</span><label className="flex cursor-pointer select-none items-center gap-2 text-xs text-muted-foreground hover:text-foreground"><input type="checkbox" className="size-3.5 rounded border border-input accent-foreground" checked={showHidden} onChange={e=>{setShowHidden(e.target.checked);try{window.localStorage.setItem('bb:file-gateway:show-hidden',String(e.target.checked));}catch{}}}/>Скрытые файлы и папки</label></div>
  </div>
