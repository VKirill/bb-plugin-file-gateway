...
function Folder({machine,path,label,initialOpen=false,showHidden}:{machine:Machine;path:string;label:string;initialOpen?:boolean;showHidden:boolean}){
 ...
 async function preview(target:string){setError('');try{const ref=await rpc.call('preview',{hostId:machine.id,path:target,...(threadId?{threadId}:{})});if(!navigate.experimental_openFilePreview({target:{kind:'host',...ref},location:null}))throw new Error(threadId?'Не удалось открыть просмотр файла':'Для просмотра файла откройте существующий чат');}catch(e){setError(e instanceof Error?e.message:String(e));}}
...
export function GatewayExplorer(){
 ...
 return <div className="flex h-full flex-col" aria-label="Файлы подключений">
  <div className="shrink-0 space-y-3 border-b border-border px-3 pb-3 pt-2">
   <div className="flex items-center gap-1"><Select value={selected} onValueChange={setSelected}><SelectTrigger aria-label="Сервер или FTP-подключение" className="h-9 min-w-0 flex-1 border-transparent bg-muted/40 px-2 hover:bg-state-hover"><div className="flex min-w-0 items-center gap-2"><ExplorerIcon name={selected.startsWith('remote_')?'globe':'server'} className="size-4 text-muted-foreground"/><SelectValue placeholder="Выберите подключение"/></div></SelectTrigger><SelectContent>{machines.map(m=><SelectItem key={m.id} value={m.id}>{m.name}{!m.configured?' — выключено':!['connected','configured'].includes(m.status)?' — не в сети':''}</SelectItem>)}</SelectContent></Select><Button size="sm" variant="ghost" className="size-8 shrink-0 p-0 text-muted-foreground" aria-label="Обновить подключения" onClick={()=>setVersion(v=>v+1)}><ExplorerIcon name="refresh"/></Button></div>
